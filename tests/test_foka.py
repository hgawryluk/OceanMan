...
def schedule(foka_fixture):
    data, url, md5 = foka_fixture
    return foka.parse(data, url, md5)
...
class TestSlotCount:
    def test_total_slot_count(self, schedule):
        # 6-lane pool, 30-min slots, 7 days — fixture: week starting 01 Jun 2026
        assert len(schedule.slots) == 224
    ...
    def test_monday_slot_count(self, schedule):
        ...
        assert len(monday) == 32

    def test_saturday_slot_count(self, schedule):
        saturday = [s for s in schedule.slots if s.weekday == "saturday"]
        assert len(saturday) == 32

    def test_sunday_slot_count(self, schedule):
        sunday = [s for s in schedule.slots if s.weekday == "sunday"]
        assert len(sunday) == 32
...
class TestSlotIntegrity:
    ...
    def test_pool_field_on_all_slots(self, schedule):
        assert all(s.pool == "foka" for s in schedule.slots)
...
class TestKnownSlotValues:
    """Anchor tests against specific slots from the fixture PDF (week starting 01 Jun 2026)."""
    ...
    def test_monday_last_slot(self, schedule):
        ...
        assert last.free_lanes == 5

    def test_saturday_first_slot(self, schedule):
        s = self._get(schedule, "saturday", "06:00")
        assert s.slot_end == "06:30"
        assert s.free_lanes == 6

    def test_saturday_and_sunday_start_at_0600(self, schedule):
        for day in ("saturday", "sunday"):
            slots = sorted(
                [s for s in schedule.slots if s.weekday == day],
                key=lambda s: s.slot_start,
            )
            assert slots[0].slot_start == "06:00"
